...
const char * const BITCOIN_CONF_FILENAME = "corvus.conf";
const char * const BITCOIN_PID_FILENAME = "corvusd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "corvus" is a composite category enabling all Corvus-related debug output
            if(ptrCategory->count(string("corvus"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CorvusCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CorvusCore
    // Mac: ~/Library/Application Support/CorvusCore
    // Unix: ~/.corvuscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CorvusCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CorvusCore";
#else
    // Unix
    return pathRet / ".corvuscore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty corvus.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override corvus.conf
